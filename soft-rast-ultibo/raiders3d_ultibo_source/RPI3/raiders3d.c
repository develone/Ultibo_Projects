 ...
 * raiders3d.c
 *  
 * A simple Game from the book Tricks of the 3D Game Programming Gurus
 * to test the book engine
 *  
 */
...
#include "t3deng/t3dlib1.h"
#include "t3deng/t3dlib4.h"
#include "t3deng/t3dlib5.h"
#include "t3deng/t3dlib6.h"
#include "t3deng/t3dlib7.h"
#include "t3deng/t3dlib8.h"
#include "t3deng/t3dlib9.h"
#include "t3deng/t3dlib10.h"
#include "t3deng/t3dlib11.h"
#include "t3deng/t3dlib12.h"
#include "t3deng/t3dlib13.h"
...
int Draw_Bitmap_Font_String(BOB_PTR font,
                            ...
                            UCHAR *video_buffer, int lpitch);//LPDIRECTDRAWSURFACE7 dest);
...
#endif
void glSwapBuffer();
//void getMousePosition(int *x, int *y, int *Buttons);
...
Draw_BOB16(font,
               video_buffer, //pointer to video buffer surface
               lpitch,
               WINDOW_WIDTH,//Form1->PaintBox1->Width ,
               WINDOW_HEIGHT);//Form1->PaintBox1->Height);
...
for (int index = 0; index < NUM_ALIENS; index++)
    ...
    switch(aliens[index].state)
          ...
          Transform_OBJECT4DV2(&obj_alien, &mrot, TRANSFORM_LOCAL_TO_TRANS,1, 0);
          ...
          if (!Cull_OBJECT4DV2(&obj_alien, &cam, CULL_OBJECT_XYZ_PLANES))
             ...
             Model_To_World_OBJECT4DV2(&obj_alien, TRANSFORM_TRANS_ONLY,0);
...
for (int eindex = 0; eindex < NUM_EXPLOSIONS; eindex++)
    ...
    if ((explobj[eindex].state & OBJECT4DV2_STATE_ACTIVE))
       ...
       // rotate the local coords of the object
       Transform_OBJECT4DV2(&explobj[eindex], &mrot, TRANSFORM_LOCAL_TO_TRANS,1, 0);

       // perform world transform
       Model_To_World_OBJECT4DV2(&explobj[eindex], TRANSFORM_TRANS_ONLY, 0);
       ...
       for (int pindex = 0; pindex < explobj[eindex].num_polys; pindex++)
           {
           // vertex 0
           VECTOR4D_Add3(&explobj[eindex].vlist_local[pindex*3 + 0].v,
                        &trajectory[pindex*2+0], 
                        &explobj[eindex].vlist_local[pindex*3 + 0].v);

           // vertex 1
           VECTOR4D_Add3(&explobj[eindex].vlist_local[pindex*3 + 1].v,
                        &trajectory[pindex*2 + 0], 
                        &explobj[eindex].vlist_local[pindex*3 + 1].v);

           // vertex 2
           VECTOR4D_Add3(&explobj[eindex].vlist_local[pindex*3 + 2].v,
                        &trajectory[pindex*2 + 0], 
...
for (int eindex = 0; eindex < NUM_EXPLOSIONS; eindex++)
    ...
    if (explobj[eindex].state == OBJECT4DV2_STATE_NULL)
       ...
       for (int pindex = 0; pindex < explobj[eindex].num_polys; pindex++)
           ...
           VECTOR4D_Scale3(rate, &trajectory[pindex*2+0], &trajectory[pindex*2+0]);          
           ...
           VECTOR4D_Add3(&mv, &trajectory[pindex*2+0], &trajectory[pindex*2+0]);
       ...
       Transform_OBJECT4DV2(&explobj[eindex], mrot, TRANSFORM_LOCAL_ONLY,1,0);
...
Load_Bitmap_File(&bitmap16bit, "cockpit03W.BMP");
Create_BOB(&cockpit, 0,0,800,480,2, BOB_ATTR_VISIBLE | BOB_ATTR_SINGLE_FRAME, 0, 0, 16);
Load_Frame_BOB16(&cockpit, &bitmap16bit,0,0,0,BITMAP_EXTRACT_MODE_ABS);
Unload_Bitmap_File(&bitmap16bit);

Load_Bitmap_File(&bitmap16bit, "cockpit03bW.BMP");
...
switch(game_state)
      ...
      //Mem_Set_WORD((USHORT*)lvideo_buffer, RGB16Bit565(0,0,0), pBackBuffer->Width * pBackBuffer->Height);

//Draw_Bitmap16C(&starscape.images[starscape.curr_frame],//font,
///               video_buffer, //pointer to video buffer surface
//               pBackBuffer->Width << 1,
//               WINDOW_WIDTH,//Form1->PaintBox1->Width ,
//               WINDOW_HEIGHT);//Form1->PaintBox1->Height);

Draw_Bitmap16C( 0, 0, starscape.images[starscape.curr_frame].width ,
                      starscape.images[starscape.curr_frame].height ,
                      starscape.images[starscape.curr_frame].buffer ,
                      lvideo_buffer, mempitch,
 ...
 getKey(&Key);
     ...
     if (Key == 'a')//keyboard_state[DIK_A])
      ...
      if (Key == 'p')//keyboard_state[DIK_P])
       ...
       //cross_x+=(PanelMouseX);
       //cross_y-=(PanelMouseY);
       cross_x = -((WINDOW_WIDTH/2 - PanelMouseX));
       cross_y = WINDOW_HEIGHT/2 - PanelMouseY;
       ...
       Draw_Starfield(lvideo_buffer,  mempitch);//back_buffer, back_lpitch);
       ...
       if (wireframe_mode  == 0)
          {
          Draw_RENDERLIST4DV2_Solid16(&rend_list, lvideo_buffer,  mempitch);//back_buffer, back_lpitch);
          }
       else
       if (wireframe_mode  == 1)
          {
          Draw_RENDERLIST4DV2_Wire16(&rend_list, lvideo_buffer,  mempitch);//back_buffer, back_lpitch);
          } // end if

       // draw energy bindings
       Render_Energy_Bindings(energy_bindings, 1, 8, 16, RGB16Bit(0,255,0), lvideo_buffer,  mempitch);//back_buffer, back_lpitch);

       // fire the weapon

       if (game_state == GAME_STATE_RUN && (ButtonsMouse == 1) && weapon_energy > 20)
          {
          ...
          Render_Weapon_Bursts(weapon_bursts, 2, 8, 16, RGB16Bit(0,255,0), lvideo_buffer,  mempitch);//back_buffer, back_lpitch);
...
Draw_BOB16(&cockpit,
               lvideo_buffer, //pointer to video buffer surface
               mempitch,
               WINDOW_WIDTH ,
               WINDOW_HEIGHT);
       ...
       Draw_Bitmap_Font_String(&tech_font, TPOS_SCORE_X,TPOS_SCORE_Y, work_string, FONT_HPITCH, FONT_VPITCH, lvideo_buffer,  mempitch);//lpddsback);

       sprintf(work_string, "HITS:%d", hits);  
       Draw_Bitmap_Font_String(&tech_font, TPOS_HITS_X,TPOS_HITS_Y, work_string, FONT_HPITCH, FONT_VPITCH, lvideo_buffer,  mempitch);//lpddsback);

       sprintf(work_string, "ESCAPED:%d", escaped);  
       Draw_Bitmap_Font_String(&tech_font, TPOS_ESCAPED_X,TPOS_ESCAPED_Y, work_string, FONT_HPITCH, FONT_VPITCH, lvideo_buffer,  mempitch);//lpddsback);

       sprintf(work_string, "LEVEL:%2.4f", diff_level);
       Draw_Bitmap_Font_String(&tech_font, TPOS_GLEVEL_X,TPOS_GLEVEL_Y, work_string, FONT_HPITCH, FONT_VPITCH, lvideo_buffer,  mempitch);//lpddsback);

       sprintf(work_string, "VEL:%3.2fK/S", player_z_vel+diff_level);
       Draw_Bitmap_Font_String(&tech_font, TPOS_SPEED_X,TPOS_SPEED_Y, work_string, FONT_HPITCH, FONT_VPITCH, lvideo_buffer,  mempitch);//lpddsback);

       sprintf(work_string, "NRG:%d", weapon_energy);
       Draw_Bitmap_Font_String(&tech_font, TPOS_ENERGY_X,TPOS_ENERGY_Y, work_string, FONT_HPITCH, FONT_VPITCH, lvideo_buffer,  mempitch);//lpddsback);
       ...
       if (game_state == GAME_STATE_OVER)
          ...
          Draw_Bitmap_Font_String(&tech_font, TPOS_GINFO_X-(FONT_HPITCH/2)*strlen(work_string),TPOS_GINFO_Y, work_string, FONT_HPITCH, FONT_VPITCH, lvideo_buffer,  mempitch);//lpddsback);

          sprintf(work_string, "PRESS ENTER OR RBUTTON TO PLAY AGAIN");
          Draw_Bitmap_Font_String(&tech_font, TPOS_GINFO_X-(FONT_HPITCH/2)*strlen(work_string),TPOS_GINFO_Y + 2*FONT_VPITCH, work_string, FONT_HPITCH, FONT_VPITCH, lvideo_buffer,  mempitch);//lpddsback);

          // logic...
        if(Key == 0)
        {
          getKey(&Key );
          if (Key == 0x4c)//keyboard_state[DIK_RETURN])
             {
             game_state = GAME_STATE_RESTART;
             } // end if
		}
       ...
       Draw_Bitmap_Font_String(&tech_font, 4,WINDOW_HEIGHT-16, work_string, FONT_HPITCH, FONT_VPITCH, lvideo_buffer, mempitch);//lpddsback);
#endif
       ...
       Draw_Bitmap_Font_String(&tech_font, TPOS_GINFO_X-(FONT_HPITCH/2)*strlen(work_string),TPOS_GINFO_Y+24, work_string, FONT_HPITCH, FONT_VPITCH, lvideo_buffer,  mempitch);//lpddsback);
#endif
...
Draw_BOB16(&crosshair,
  ...
  //video_buffer = (unsigned short*)lvideo_buffer;
  //video_buffer2;
  //for(int i = 0; i < pBackBuffer->Height; i++)
  //{
  // video_buffer2 = (unsigned short*)pBackBuffer->ScanLine[i];
  // memcpy(video_buffer2,&video_buffer[pBackBuffer->Width * i],pBackBuffer->Width << 1);//* 2);

  //}

  //Form1->PaintBox1->Canvas->Draw(0,0,pBackBuffer);
  memcpy(dest_buffer,lvideo_buffer, (ScreenWidth * 2) * (ScreenHeight * 2 ));
       ...
       // check of user is trying to exit
        /*
       if (GetAsyncKeyState(VK_ESCAPE) )//KEY_DOWN(VK_ESCAPE) || keyboard_state[DIK_ESCAPE])
          {
