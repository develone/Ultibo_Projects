...
int OBJECT4DV2_Set_Frame(OBJECT4DV2 *object4dv2,int frame)
{
...
if (!(object4dv2->attr & OBJECT4DV2_ATTR_MULTI_FRAME))
   return(0);
...
if (frame >= object4dv2->num_frames)
    frame = object4dv2->num_frames - 1;

// set frame
object4dv2->curr_frame = frame;
...
object4dv2->vlist_local = &(object4dv2->head_vlist_local[frame*object4dv2->num_vertices]);
object4dv2->vlist_trans = &(object4dv2->head_vlist_trans[frame*object4dv2->num_vertices]);
...
VECTOR4D_Add3(&obj->world_pos, vt, &obj->world_pos);
...
if (!all_frames)
   ...
   if (coord_select == TRANSFORM_LOCAL_TO_TRANS)
      ...
      for (int vertex=0; vertex < obj->num_vertices; vertex++)
          {
          // translate vertex
          VECTOR4D_Add3(&obj->vlist_local[vertex].v, &obj->world_pos, &obj->vlist_trans[vertex].v);
          // copy normal
      ...
      for (int vertex=0; vertex < obj->num_vertices; vertex++)
          {
          // translate vertex
          VECTOR4D_Add3(&obj->vlist_trans[vertex].v, &obj->world_pos, &obj->vlist_trans[vertex].v);
          } // end for vertex
      } // end else trans
   ...
   if (coord_select == TRANSFORM_LOCAL_TO_TRANS)
      ...
      for (int vertex=0; vertex < obj->total_vertices; vertex++)
          {
          // translate vertex
          VECTOR4D_Add3(&obj->head_vlist_local[vertex].v, &obj->world_pos, &obj->head_vlist_trans[vertex].v);
          // copy normal
      ...
      for (int vertex=0; vertex < obj->total_vertices; vertex++)
          {
          // translate vertex
          VECTOR4D_Add3(&obj->head_vlist_trans[vertex].v, &obj->world_pos, &obj->head_vlist_trans[vertex].v);
          } // end for vertex
      } // end else trans
       ...
       while(1)
            ...
            if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['Shader'] ['name:'] ['texture'] ['map']") )
               ...
               while(1)
                    ...
                    if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['file'] ['name:'] ['string']") )
                       {
                       ...
                       memcpy(materials[material_index + num_materials].texture_file, &parser.bufferstruct[18], strlen(parser.bufferstruct) - 18 + 2 );
...
for (int poly=0; poly < obj->num_polys; poly++)
    ...
    if (tmp_string[0] == '0' && toupper_r(tmp_string[1]) == 'X')
...
for (int poly=0; poly < obj->num_polys; poly++)
    ...
    if (obj->plist[poly].attr & POLY4DV2_ATTR_SHADE_MODE_GOURAUD)
       ...
       VECTOR4D_Add3(&obj->vlist_local[vindex_0].n, &n, &obj->vlist_local[vindex_0].n);
       VECTOR4D_Add3(&obj->vlist_local[vindex_1].n, &n, &obj->vlist_local[vindex_1].n);
       VECTOR4D_Add3(&obj->vlist_local[vindex_2].n, &n, &obj->vlist_local[vindex_2].n);
       } // end for poly
...
for (int vertex = 0; vertex < obj->num_vertices; vertex++)
    ...
    if (polys_touch_vertex[vertex] >= 1)
       ...
       VECTOR4D_Normalize1(&obj->vlist_local[vertex].n);
