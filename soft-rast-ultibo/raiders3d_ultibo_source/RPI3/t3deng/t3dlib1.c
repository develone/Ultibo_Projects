...
{
  Create_Bitmap(&bob->images[index],x,y,width, height,bpp);
  bob->images[index].color_key_value  = color_key_value;
}
...
if ((bob->attr & BOB_ATTR_CLONE))
    ...
    for (index=0; index<MAX_BOB_FRAMES; index++)
        if (bob->images[index].buffer)
            Destroy_Bitmap(&bob->images[index]); //(bob->images[index])->Release();
...
if (!(bob->attr & BOB_ATTR_VISIBLE))
 ...
 Draw_Bitmap16B( bob->x, bob->y, bob->width , bob->height , bob->images[bob->curr_frame].buffer ,
                 video_buffer, lpitch, screen_width, screen_height, bob->images[bob->curr_frame].color_key_value );
/*
...
//RECT dest_rect,   // the destination rectangle
//     source_rect; // the source rectangle
 if(bob->images[bob->curr_frame].buffer != NULL)
 {

   BITMAP_IMAGE Scaling_dest;

   float width_sf;
   float height_sf;

   width_sf =  (float)bob->width / swidth;
   width_sf = 1/width_sf;
   height_sf = (float)bob->height /sheight;
   height_sf = 1/height_sf;
...
if (!(bob->attr & BOB_ATTR_VISIBLE))
 ...
 Create_Bitmap(&Scaling_dest, bob->x , bob->y , swidth , sheight, 16);
 ...
 int    lpitch_2 = Scaling_dest.width; // lpitch in USHORT terms


 dest_addr = ((USHORT *)Scaling_dest.buffer);// + source_bitmap->y*lpitch_2 + source_bitmap->x;
 source_addr = ((USHORT *) bob->images[bob->curr_frame].buffer );

 //Draw_Bitmap16S( bob->x, bob->y, bob->width, bob->height, swidth , sheight , bob->images[bob->curr_frame].buffer ,
 //                video_buffer, lpitch, screen_width, screen_height, bob->images[bob->curr_frame].color_key_value );

 USHORT pixel;
   for (int index_y = 0; index_y < sheight/height_sf; index_y++)
   {
 for(int scale_y = 0; scale_y < height_sf; scale_y++)
 {
      // inner loop, where the action takes place
     for (int index_x = 0, index_xd = 0; index_x < swidth/width_sf; index_x++)
     {


        // read pixel from source bitmap,
           //video_buffer[index_x] = pixel;
         for(int scale_x = 0; scale_x < width_sf; scale_x++)
         {
          if((pixel = source_addr[index_x]));//bitmap[index_x]))
          {
           dest_addr[index_xd] = pixel;
          }
          index_xd++;
         }
       ...
       // advance pointers
       //video_buffer+=lpitch; // bytes per scanline
   ...
   source_addr += bob->images[bob->curr_frame].width; //width;
    //dest_addr+= lpitch_2;
    //dest_addr+= lpitch_2;
    }


 Draw_Bitmap16B( bob->x , bob->y, //position to draw bitmap
                   swidth, sheight, //size of bitmap in pixels
                   Scaling_dest.buffer, //pointer to bitmap data
                   video_buffer, //pointer to video buffer surface
                   lpitch,
                   screen_width,
                   screen_height,
                   bob->images[bob->curr_frame].color_key_value);



 Destroy_Bitmap(&Scaling_dest);
 }
/*
// fill in the destination rect
dest_rect.left   = bob->x;
...
dest_ptr = (USHORT *)bob->images[frame].buffer;
...
for (int index_y=0; index_y<bob->height; index_y++)
    ...
    // advance pointers
    dest_ptr   += bob->images[frame].width; //(ddsd.lPitch >> 1);
    source_ptr += bitmap->bitmapinfoheader.biWidth;
    } // end for index_y
...
dd_pixel_format = DD_PIXEL_FORMAT565;//RGB16Bit565;//ddpf.dwRGBBitCount;
...
Load_Palette_From_File((char*)DEFAULT_PALETTE_FILE, palette);
 }
...
sprintf(timestring, "time");//, timeline, timebuffer.millitm, &timeline[20]);
...
if (bitmap->bitmapinfoheader.biBitCount==8 || bitmap->bitmapinfoheader.biBitCount==16) 
   ...
   if (bitmap->buffer)
       free(bitmap->buffer);
printf("16 Bit malloc\n");
   // allocate the memory for the image
   if (!(bitmap->buffer = (UCHAR *)malloc(bitmap->bitmapinfoheader.biSizeImage)))
      {
		  printf("16 Bit malloc failed\n");
      // close the file
      //_lclose(file_handle);
	  fclose(file_handle);
...
///////////////////////////////////////////////////////////
void Draw_Bitmap16C(int x, int y, //position to draw bitmap
                   ...
                   int transparent)
{
   ...
   //int index,           // looping vars
   //    pixel_x,
   int    lpitch_2 = lpitch >> 1; // lpitch in USHORT terms

   // compute starting destination address
   dest_addr = ((USHORT *)video_buffer);// + source_bitmap->y*lpitch_2 + source_bitmap->x;
   source_addr = ((USHORT *)bitmap);
   ...
   // compute starting address in video_buffer
     //video_buffer += (x1 + y1*lpitch);
     dest_addr  += (x1 + y1*lpitch_2);
   // compute starting address in bitmap to scan data from
     //bitmap += (x_off + y_off*width);
     source_addr += (x_off + y_off*width);
   ...
   USHORT pixel; // used to read/write pixels
   if(transparent)
   {
...
///////////////////////////////////////////////////////////
void Draw_Bitmap16B(int x, int y, //position to draw bitmap
                   int width, int height, //size of bitmap in pixels
                   UCHAR *bitmap, //pointer to bitmap data
                   UCHAR *video_buffer, //pointer to video buffer surface
                   int lpitch,
...
///////////////////////////////////////////////////////////
void Draw_Bitmap16S(int x, int y, //position to draw bitmap
                   int width, int height, //size of bitmap in pixels
                   int swidth, int sheight, //size of bitmap in pixels scale
                   UCHAR *bitmap, //pointer to bitmap data
                   UCHAR *video_buffer, //pointer to video buffer surface
                   int lpitch,
                   int screen_width,
                   ...
                   USHORT colorkey)
{
   ...
   width_sf =  (float)width / swidth;
   width_sf = 1/width_sf;
   height_sf = (float)height/sheight;
   height_sf = 1/height_sf;


   // first do trivial rejections of bitmap, is it totally invisible?
   if ((x >= screen_width) || (y>= screen_height) ||
       ((x + swidth) <= 0) || ((y + sheight) <= 0))
         return;
   ...
   // compute starting address in bitmap to scan data from
     ...
     //Form1->Label3->Caption = dx;
     //Form1->Label4->Caption = swidth - x;
   ...
   //Form1->Label2->Caption = 0.9999 * width_sf

   for (int index_y = 0; index_y < dy/height_sf; index_y++)
   {
 for(int scale_y = 0; scale_y < height_sf; scale_y++)
 {
      // inner loop, where the action takes place
     for (int index_x = 0, index_xd = 0; index_x < dx/width_sf; index_x++)
     {
        ...
        // read pixel from source bitmap,
         ...
         for(int scale_x = 0; scale_x < width_sf; scale_x++)
         {
          if ((pixel = source_addr[index_x]) && (pixel != colorkey));//bitmap[index_x]))
          {
           dest_addr[index_xd] = pixel;
          }
          index_xd++;
