...
// reset file system ////////////////////////////////////////////
//int CPARSERV1::Reset()
int CPARSERV1_Reset(CPARSERV1 *parser)
{
...
if (parser->fstream)
   fclose(parser->fstream);

parser->fstream = NULL;

// clear and reset buffer
memset(parser->bufferstruct, 0, sizeof(parser->bufferstruct ));

parser->length    = 0;
parser->num_lines = 0;

// set comment
strcpy(parser->comment, PARSER_DEFAULT_COMMENT);
...
//CPARSERV1::CPARSERV1()
void CPARSERV1_Init(CPARSERV1 *parser)
{
#ifdef PARSER_DEBUG_ON
...
//CPARSERV1::~CPARSERV1()
void CPARSERV1_Delete(CPARSERV1 *parser)
{
#ifdef PARSER_DEBUG_ON
...
int CPARSERV1_Open(CPARSERV1 *parser, char *filename)
{
#ifdef PARSER_DEBUG_ON
printf("\nEntering Open().");
#endif
...
// opens a file
if ((parser->fstream = fopen(filename, "r"))!=NULL)
   {
...
// close file ////////////////////////////////////////////////////
//int CPARSERV1::Close()
int CPARSERV1_Close(CPARSERV1 *parser)
{
return(CPARSERV1_Reset(parser));
} // end Close

// get line //////////////////////////////////////////////////////

char *CPARSERV1_Getline(CPARSERV1 *parser, int mode)
{
#ifdef PARSER_DEBUG_ON
printf("\nEntering Getline().");
#endif
...
char *string;
//Form1->Memo1->Lines->Add("ENTERING GETLINE");
...
if (parser->fstream )
   ...
   if (mode & PARSER_STRIP_EMPTY_LINES)
       ...
       while(1)
            ...
            if ((string = fgets(parser->bufferstruct ,PARSER_BUFFER_SIZE, parser->fstream ))==NULL)
               break;
            ...
            if ((slength - sindex) > 0)
               ...
               memmove((void *)parser->bufferstruct, (void *)&string[sindex],  (slength - sindex)+1 );
               string = parser->bufferstruct;
               ...
               if (mode & PARSER_STRIP_COMMENTS)
                   ...
                   char *comment_string = strstr(string, parser->comment);
       ...
       string = fgets( parser->bufferstruct ,PARSER_BUFFER_SIZE, parser->fstream);
       } // end else 
   ...
   if (string)
      ...
      if (mode & PARSER_STRIP_WS_ENDS)
         {
         StringLtrim(parser->bufferstruct);
         StringRtrim(parser->bufferstruct);
         } // end if         

      // compute line length
      parser->length = strlen(parser->bufferstruct);
...
int CPARSERV1_SetComment(CPARSERV1 *parser,char *string)
{
// sets the comment string
if (strlen(string) < PARSER_MAX_COMMENT)
   {
   strcpy(parser->comment, string);
   return(1);
   } // end if
else
   return(0);
...
int CPARSERV1_Pattern_Match(CPARSERV1 *parser, char *string, char *pattern, ...)
{
...
while(1)
    {
    // eat whitepace
    while(isspace_r(pattern[tok_start]) )
          tok_start++;
...
printf("\nFound Literal token = %s",token_string[num_tokens]);
             ...
             if (pattern[tok_start+2] == '=' || 
                ...
                while( isdigit_r(pattern[tok_end] ) )
                      tok_end++;
...
while(1)
      ...
      case PATTERN_STATE_INIT:
           ...
           parser->num_pints = parser->num_pfloats = parser->num_pstrings = 0;
      ...
      case PATTERN_STATE_NEXT:
           ...
           tok_start = tok_end;
           while(isspace_r(buffer[tok_start])) tok_start++;
           tok_end = tok_start;

           while(!isspace_r(buffer[tok_end]) && tok_end < strlen(buffer)) tok_end++;
      ...
      case PATTERN_STATE_FLOAT:
           ...
           if (f!=FLT_MIN)
              {
              parser->pfloats[parser->num_pfloats++] = f;
      ...
      case PATTERN_STATE_INT:
           ...
           if (i!=INT_MIN)
              {
              parser->pints[parser->num_pints++] = i;
      ...
      case PATTERN_STATE_LITERAL:
           ...
           if (strcmp(token, token_string[curr_tok]) == 0)
              ...
              strcpy(parser->pstrings[parser->num_pstrings++], token);
...
while(string_in[index_in])
     {
     for (sindex = 0; sindex < slength; sindex++)
         if (toupper_r(string_in[index_in]) == toupper_r(replace_chars[sindex]))
            {
            // replace it
            string_out[index_out++] = rep_char;
            index_in++;
...
while(string_in[index_in])
     {
     for (sindex = 0; sindex < slength; sindex++)
         if (toupper_r(string_in[index_in]) == toupper_r(strip_chars[sindex]))
            {
            // jump over input char, it's stripped
            index_in++;
            num_extracts++;
...
// must be of the form
// [whitespace] 
while(isspace_r(*string)) string++;

// [sign] 
if (*string=='+' || *string=='-') string++;

// [digits] 
while(isdigit_r(*string)) string++;
...
while( isspace_r(string[sindex]) && sindex >= 0)
     string[sindex--] = 0;
...
while(isspace_r(string[sindex]) && sindex < slength)
...
//if (!parser.Open(filename)) CPARSERV1_
if (!CPARSERV1_Open(&parser,filename))
   {
...
while(1)
     ...
     if (!CPARSERV1_Getline(&parser,PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
        {
     ...
     if (CPARSERV1_Pattern_Match(&parser, parser.bufferstruct, "['Named'] ['object:']"))
        {
        // at this point we have the string with the name in it, parse it out by finding 
        // name between quotes "name...."
        strcpy(token_buffer, parser.bufferstruct);
        strcpy(seps, "\"");        
...
while(1)
     ...
     // check for pattern?  
     if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['Tri-mesh,'] ['Vertices:'] [i] ['Faces:'] [i]"))
        {
...
while(1)
     {
     // get next line
     //if (!parser.Getline(PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))CPARSERV1_
     if (!CPARSERV1_Getline(&parser,PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
        {
     ...
     if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['Vertex'] ['list:']"))
        {
...
// "Vertex: d  X:d.d Y:d.d  Z:d.d"
 for (int vertex = 0; vertex < obj->num_vertices; vertex++)
     ...
     StripChars(parser.bufferstruct, parser.bufferstruct, ":XYZ", 1);
     
     // check for pattern?  
     if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['Vertex'] [i] [f] [f] [f]"))
        {
...
while(1)
     ...
     // check for pattern?  
     if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['Face'] ['list:']"))
        {
...
for (int poly=0; poly < obj->num_polys; poly++)
    ...
    // since the output is guaranteed to be the same length or shorter as the input :)
    StripChars(parser.bufferstruct, parser.bufferstruct, ":ABC",1);
     
    // check for pattern?  
    if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['Face'] [i] [i] [i] [i]"))
       {
    ...
    // which we can parse!
    ReplaceChars(parser.bufferstruct, parser.bufferstruct, ":\"rgba", ' ',1);
     
    // check for pattern?  
    if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "[i] [i] [i]"))
       {
...
// return success
CPARSERV1_Close(&parser);
CPARSERV1_Delete(&parser);
return(1);
...
while(1)
     ...
     // check for pattern?  
     if ( CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['Name'] [s>0]") )
        {
...
while(1)
     ...
     // check for pattern?  
     if ( CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['center'] [f] [f] [f]") )
        {
        ...
        // "x axis" 
        CPARSERV1_Getline(&parser,PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS);
        CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['x'] ['axis'] [f] [f] [f]");
        ...
        // "y axis" 
        CPARSERV1_Getline(&parser,PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS);
        CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['y'] ['axis'] [f] [f] [f]");
        ...
        // "z axis" 
        CPARSERV1_Getline(&parser,PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS);
        CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['z'] ['axis'] [f] [f] [f]");
...
while(1)
     ...
     // check for pattern?  
     if ( CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['Transform']") )
        {

        // "x axis" 
        CPARSERV1_Getline(&parser,PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS);
        CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "[f] [f] [f]");
...
while(1)
     ...
     // check for pattern?  
     if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['World'] ['Vertices'] [i]") )
        {
...
// "d.d d.d d.d"
 for (int vertex = 0; vertex < obj->num_vertices; vertex++)
     ...
     // check for pattern?  
     if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "[f] [f] [f]"))
        {
        // at this point we have the x,y,z in the the pfloats array locations 0,1,2
        obj->vlist_local[vertex].x = parser.pfloats[0];
        obj->vlist_local[vertex].y = parser.pfloats[1];
...
while(1)
     ...
     // check for pattern?
     if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['Texture'] ['Vertices'] [i]") )
        {
...
// "d.d d.d"
 for (int tvertex = 0; tvertex < num_texture_vertices; tvertex++)
     ...
     // check for pattern?  
     if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "[f] [f]"))
        {
...
while(1)
     ...
     // check for pattern?  
     if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['Faces'] [i]"))
        {
...
for (int poly=0; poly < obj->num_polys; poly++)
    ...
    while(1)
         ...
         // check for pattern?  
         if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['Face'] ['verts'] [i] ['flags'] [i] ['mat'] [i]"))
            {
           ...
           ReplaceChars(parser.bufferstruct, parser.bufferstruct, ",<>",' ',1);
           CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "[i] [i] [i] [i] [i] [i]");
...
for (int curr_material = 0; curr_material < num_materials_object; curr_material++)
    ...
    // check for pattern?  
    if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['mat#'] [i]") )
       {
       ...
       while(1)
            ...
            if (!CPARSERV1_Getline(&parser,PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
               ...
               if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['rgb'] [f] [f] [f]") )
                  {
       ...
       while(1)
            ...
            if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['alpha'] [f] ['ka'] [f] ['ks'] [f] ['exp'] [f]") )
               {
       ...
       while(1)
            ...
            if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['Shader'] ['class:'] ['color']") )
               {
               break;
               } // end if
       ...
       while(1)
            ...
            if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['Shader'] ['name:'] ['plain'] ['color']") )
               {
            ...
            if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['Shader'] ['name:'] ['texture'] ['map']") )
               {
               ...
               while(1)
                    ...
                    if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['file'] ['name:'] ['string'] [s>0]") )
                       {
       ...
       while(1)
            ...
            if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['Shader'] ['class:'] ['reflectance']") )
               {
        ...
        while(1)
            ...
            if (CPARSERV1_Pattern_Match(&parser,parser.bufferstruct, "['Shader'] ['name:'] [s>0]" ) )
               {
...
if (_dir)
   ...
   // normalize it
   VECTOR4D_Normalize1(&lights[index].dir);
