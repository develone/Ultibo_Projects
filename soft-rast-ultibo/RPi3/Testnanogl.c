...
DDraw_Init(ScreenWidth, ScreenHeight, WINDOW_BPP, WINDOWED_APP);
...
Init_CAM4DV1(&cam,            // the camera object
             ...
             ScreenWidth,    // size of final screen viewport
             ScreenHeight);
...
memset(lvideo_buffer, 0 , (ScreenWidth * 2) * (ScreenHeight * 2 ));
// draw the sky
...
Transform_OBJECT4DV2(&obj_terrain, &mrot, TRANSFORM_LOCAL_TO_TRANS,1,0);

// perform world transform
Model_To_World_OBJECT4DV2(&obj_terrain, TRANSFORM_TRANS_ONLY,0);
...
Transform_OBJECT4DV2(&obj_model, &mrot, TRANSFORM_LOCAL_TO_TRANS,1,0);

// perform world transform
Model_To_World_OBJECT4DV2(&obj_model, TRANSFORM_TRANS_ONLY,0);
...
// transform the local coords of the object
Transform_OBJECT4DV2(&obj_light_array[INDEX_RED_LIGHT_INDEX], &mrot, TRANSFORM_LOCAL_TO_TRANS,1,0);

// perform world transform
Model_To_World_OBJECT4DV2(&obj_light_array[INDEX_RED_LIGHT_INDEX], TRANSFORM_TRANS_ONLY,0);
...
// transform the local coords of the object
Transform_OBJECT4DV2(&obj_light_array[INDEX_YELLOW_LIGHT_INDEX], &mrot, TRANSFORM_LOCAL_TO_TRANS,1,0);

// perform world transform
Model_To_World_OBJECT4DV2(&obj_light_array[INDEX_YELLOW_LIGHT_INDEX], TRANSFORM_TRANS_ONLY,0);
...
// draw background
//Draw_Bitmap16C(&background_bmp, lvideo_buffer, mempitch,ScreenWidth, ScreenHeight,1);
Draw_Bitmap16C( 0, 0, background_bmp.width , background_bmp.height, background_bmp.buffer, lvideo_buffer, mempitch,ScreenWidth, ScreenHeight,1);
...
if (wireframe_mode  == 0)
   Draw_RENDERLIST4DV2_Wire16(&rend_list, lvideo_buffer, mempitch);
else
if (wireframe_mode  == 1)
   {
   ...
   rc.video_buffer   = lvideo_buffer;
   rc.lpitch         =mempitch;
   rc.mip_dist       = 0;
   rc.zbuffer        = (UCHAR *)zbuffer.zbuffer;
   rc.zpitch         = ScreenWidth*4;
...
for (int vertex=0; vertex < obj_model.num_vertices; vertex++)
    ...
    VECTOR4D_Add3(&obj_model.vlist_local[vertex].v, &obj_model.world_pos, &vi);
