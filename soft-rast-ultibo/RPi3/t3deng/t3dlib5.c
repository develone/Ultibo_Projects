...
int isspace_r(int c)
{
  ...
  //'\r'	(0x0d)	carriage return (CR)
   ...
   return c == ' ' || c == '\t' || c == '\n' || c == '\f' || c == '\r' || c == '\v'; // || whatever other char you consider space
}


int toupper_r(int ch)
{
        if(ch >= 'a' && ch <= 'z')
                return ('A' + ch - 'a');
        else
...
while(1)
    ...
    // kill the whitespace
    for (length = strlen(buffer), index = 0; isspace_r(buffer[index]); index++);
...
if (!(fp = fopen(filename, "r")))
   {
   Write_Error((char*)"Couldn't open PLG file %s.", filename);
   return(0);
   } // end if
...
if (!(token_string = Get_Line_PLG(buffer, 255, fp)))
   {
   Write_Error((char*)"PLG file error with file %s (object descriptor invalid).",filename);
   return(0);
   } // end if

Write_Error((char*)"Object Descriptor: %s", token_string);
...
for (int vertex = 0; vertex < obj->num_vertices; vertex++)
    ...
    if (!(token_string = Get_Line_PLG(buffer, 255, fp)))
          {
          Write_Error((char*)"PLG file error with file %s (vertex list invalid).",filename);
          return(0);
          } // end if
...
for (int poly=0; poly < obj->num_polys; poly++)
    ...
    if (!(token_string = Get_Line_PLG(buffer, 255, fp)))
        {
        Write_Error((char*)"PLG file error with file %s (polygon descriptor invalid).",filename);
        return(0);
        } // end if
...
for (int vertex=0; vertex < obj->num_vertices; vertex++)
    {
    // translate vertex
    VECTOR4D_Add3(&obj->vlist_local[vertex], &obj->world_pos, &obj->vlist_trans[vertex]);
    } // end for vertex
} // end if local
else
...
for (int vertex=0; vertex < obj->num_vertices; vertex++)
    {
    // translate vertex
    VECTOR4D_Add3(&obj->vlist_trans[vertex], &obj->world_pos, &obj->vlist_trans[vertex]);
    } // end for vertex
} // end else trans
...
if (coord_select == TRANSFORM_LOCAL_TO_TRANS)
   ...
   for (int vertex = 0; vertex < 3; vertex++)
       {
       // translate vertex
       VECTOR4D_Add3(&curr_poly->vlist[vertex], world_pos, &curr_poly->tvlist[vertex]);
       } // end for vertex
...
for (int poly = 0; poly < rend_list->num_polys; poly++)
   ...
   for (int vertex = 0; vertex < 3; vertex++)
       {
       // translate vertex
       VECTOR4D_Add3(&curr_poly->tvlist[vertex], world_pos, &curr_poly->tvlist[vertex]);
       } // end for vertex
...
// Step 3: u = (v x n)
VECTOR4D_Cross3(&cam->v,&cam->n,&cam->u);

// Step 4: v = (n x u)
VECTOR4D_Cross3(&cam->n,&cam->u,&cam->v);

// Step 5: normalize all vectors
VECTOR4D_Normalize1(&cam->u);
VECTOR4D_Normalize1(&cam->v);
VECTOR4D_Normalize1(&cam->n);
