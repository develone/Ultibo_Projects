...
void rb_block(unsigned char *in, int *pstart, int *pend) {
    int i,j;
    //offset = *pstart*512;
...
int main(void)
{
    ...
    char server_message[2047], client_message[2000];
    char *inp1, *inp2, *inp3;
    unsigned char *inbuf;
	...
	pgmReadFile(inp1, inbuf,pncols, pnrows);
	printf("0x%x %d 0x%x %d 0x%x \n",pstart,start,pend,end,inbuf);
	for(k=0;k<128;k++) {
	    
	    rb_block(inbuf,pstart,pend);
	    start += 4;
	    end += 4;
	    inbuf += 2048;
	}
