...
-------------
 ...
 characters LIL recognizes as special characters are
     ...
     o - inserts the { character
     c - inserts the } character
 ...
 words, etc - with the exception that line breaks inside LIL lists are
 ignored instead of ending the current command, so LIL lists can be
 specified over multiple lines in the source code without needing an escape
 prefix) where each word in the list specifies an argument and the code
 ...
 character at the end of the line with at least a single space in front
 of it, like
...
3.1. Standard LIL functions
 ...
 LIL scripts (in order of appearance in the lil.c file):
     ...
     exit [code]
       ...
       back control and handles the request
...
4.2. Execute LIL code
 ...
 has the following signature:
   ...
   When implementing a function that can loop code you may want to check if
 the code execution was interrupted - e.g. via a call to return - using the
 lil_break_run function with the following signature:

     int lil_break_run(lil_t lil,
                       int dobreak)

   The function will return 0 if the execution wasn't stopped and any other
 value if it was.  The dobreak parameter can be used to alter the break
 (interrupt) state for functions that want to do such an interruption
 themselves: a non-zero value will cause a break and zero will only return
