...
uses
Console, Classes,gpio,mmc,http,framebuffer,Network,Winsock2,font,wifidevice,strUtils,SysUtils,devices,Platform;

procedure SetWiFiWindow(AwindowHandle : THandle);
procedure WaitForIP;
...
var
  ...
  begin
    ...
    ConsoleWindowWriteln(lcltopwindow, 'Waiting for file system...');
    while not directoryexists('c:\') do
    begin
      Sleep(0);
    end;
    ConsoleWindowWriteln(lcltopwindow, 'File system ready. Initialize Wifi Device.');
    ...
    ConsoleWindowWriteln(lcltopwindow, 'Waiting for Wifi Device to be opened.');
    ...
    begin
      ConsoleWindowWriteln(lcltopwindow, 'Performing a WIFI network scan...');
      ScanResultList := TStringList.Create;

      WirelessScan(@WIFIScanCallback);

      for i := 0 to ScanResultList.Count-1 do
        ConsoleWindowWriteln(lcltopwindow, 'Found access point: ' + ScanResultList[i]);

      ScanResultList.Free;
    end
    else
      ConsoleWindowWriteln(lcltopwindow, 'Network scan not enabled in cmdline.txt (add the WIFISCAN=1 entry)');
    ...
    ConsoleWindowWriteln(lcltopwindow, 'Attempting to join WIFI network ' + SSID + ' (Country='+Country+')');

    if (Key = '') then
      ConsoleWindowWriteln(lcltopwindow, 'Warning: Key not specified - expecting the network to be unencrypted.');

    if (SSID = '') or (Country='') then
       ConsoleWindowWriteln(lcltopwindow, 'Cant join a network without SSID, Key, and Country Code.')
    else
    begin
      if (BSSIDStr <> '') then
      begin
        ConsoleWindowWriteln(lcltopwindow, 'Using BSSID configuration ' + BSSIDStr + ' from cmdline.txt');
        ...
        ConsoleWindowWriteln(lcltopwindow, 'Letting the Cypress firmware determine the best network interface from the SSID');
      ...
      begin
        ...
        ConsoleWindowWriteln(lcltopwindow, 'Network joined, waiting for an IP address...');
      ...
      begin
        ConsoleWindowWriteLn(lcltopwindow,'Failed to join the WIFI network. Status='+inttostr(status));
        ConsoleWindowWriteln(lcltopwindow, 'Waiting for auto retry...');
    ...
    on e : exception do
      ConsoleWindowWriteln(lcltopwindow, 'Exception: ' + e.message + ' at ' + inttohex(longword(exceptaddr), 8));
  end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ConsoleWindowWriteLn(lcltopwindow, 'IP address='+Winsock2TCPClient.LocalAddress);
