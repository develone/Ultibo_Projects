...
begin
  ...
  try
    ...
    begin
      ...
      ActivityLEDEnable;
      ConsoleWindowWriteLn (topwindow, 'Welcome to PIco Test Program.');
      oldState := stUnknown;
      state := stFind;
      Count := 0;
      Characters := '';
    ...
    begin
      if state <> oldState then
        begin
          ...
          ConsoleWindowWriteLn (topwindow, 'Serial Port State ' + s);
          oldState := state;
        end;
      case state of
        stFind :
          begin
            ...
            ConsoleWindowWriteLn (topwindow, 'trying to find USB CDC ACM Serial');
		...
		begin
			characters := '1';
			Count := 1;
                        ConsoleWindowWriteLn (topwindow, 'turning on led');
			SerialDeviceWrite (SerialDevice, PChar (Characters),Length (Characters), SERIAL_WRITE_NONE, Count);

			Sleep(5000);
			characters := '0';
                        ConsoleWindowWriteLn (topwindow, 'turning off led');
			SerialDeviceWrite (SerialDevice, PChar (Characters),Length (Characters), SERIAL_WRITE_NONE, Count);
