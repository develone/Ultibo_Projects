...
typedef struct svgContainer {
 ...
 /*svgContainer svgFileLoadTest; removing for GUI*/
...
void svgTessellator_main()
{
 ...
 ortho[1][1] /= (GLfloat)ScreenHeight;	
    ...
    /*LoadsvgFile( &svgFileLoadTest, VBO1, VBO2, EBO1, "example\\23ftCenter.svg" );removing for GUI*/
     ///--------------------------------------------nuklear init
...
GLfloat rotate[4][4] = {
    ...
    rotate[1][1] = cos(rad_angle);		
		...
		/*rotatesvgCont(&svgFileLoadTest, rotation);removing for GUI*/

        /*translatesvgCont(&svgFileLoadTest, (GLfloat)PanelMouseX, (GLfloat)PanelMouseY);removing for GUI*/
		...
		/*glUniformMatrix4fv(mulTrans, 1, GL_FALSE, &svgFileLoadTest.transforms[0][0]);removing for GUI*/
		...
		/*glDrawElements(GL_TRIANGLES,svgFileLoadTest.start , GL_UNSIGNED_SHORT,(void*)svgFileLoadTest.end);removing for GUI*/
        glUseProgram(0);
...
MainLoop(void* loopArg){
    ...
    if (nk_begin(ctx, "Calculator", nk_rect(260, 50, 200, 200),
        /*NK_WINDOW_BORDER|*/NK_WINDOW_MOVABLE|NK_WINDOW_SCALABLE|
        /*NK_WINDOW_CLOSABLE|*/NK_WINDOW_MINIMIZABLE|NK_WINDOW_TITLE))
    {
